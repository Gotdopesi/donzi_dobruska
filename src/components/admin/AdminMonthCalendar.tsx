...
import { ChevronLeft, ChevronRight, Clock, Mail, Phone, Trash2, User, X } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <DayDetailList rows={selectedRows} readOnly={readOnly} onDelete={onDelete} />
        </DialogContent>
      </Dialog>
    </div>
  );
...
function DayDetailPanel({
  ...
  return (
    <div className="rounded-xl border border-border bg-card/80 p-5 shadow-sm">
      ...
      <DayDetailList rows={rows} readOnly={readOnly} onDelete={onDelete} />
    </div>
  );
}
